...
double KDE::pdf(double x){
    ...
    for(unsigned int i = 0; i < data_matrix[0].size(); i++){
        double a = 1.0;
        double z = (x - data_matrix[0][i])/bandwidth;
        a *= exp(-0.5*z*z)/(bandwidth*sqrt( 2.0*M_PI));
        d += a;
    }
    return(d/data_matrix[0].size());
}	
...
//  	default_bandwidth_map[curr_var] = b;
    ...
    if(bandwidth_opt_type == 1) bandwidth_map[curr_var] = 1;
    default_bandwidth_map[curr_var] = 1;
}
...
inline double KDE::gauss_pdf(double x, double m, double s){
